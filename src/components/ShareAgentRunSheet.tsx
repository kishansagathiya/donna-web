...
import { createAgentRunShare, revokeAgentRunShare } from "../services/agentsApi";
...
export function ShareAgentRunSheet({ open, runId, goal, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    void createAgentRunShare(runId)
      .then((share) => {
  ...
  async function handleRevoke() {
    if (!runId) return;
    if (
      !window.confirm("Stop sharing? Anyone with the link will lose access.")
    ) {
      return;
    }
    setRevoking(true);
    setError(null);
    try {
      await revokeAgentRunShare(runId);
      onClose();
    } catch (err) {
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-donna-muted">
          Anyone with the link can view the prompt and output
          {goal ? (
            <>
              {" "}
              for{" "}
              <span className="font-medium text-donna-text">{goal}</span>
            </>
          ) : (
            " of this agent"
          )}
          . Steps and memory are not shared. They do not need a Donna account.
        </p>
